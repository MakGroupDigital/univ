...
import { Ecole, Stats } from './types';
import { COLORS } from './constants';

interface EcolesSectionProps {
  data: Ecole[];
  stats: Stats;
  onEdit: (id: number, champ: keyof Ecole, valeur: string) => void;
  onDelete: (id: number) => void;
}

export const EcolesSection: React.FC<EcolesSectionProps> = ({
  data,
  stats,
  onEdit,
  onDelete
        ...
        <div className="overflow-x-auto">
          <table className="w-full">
            ...
            <tbody className="divide-y divide-slate-50">
              {data.map((item, idx) => (
                <EcoleRow key={item.id} item={item} idx={idx} onEdit={onEdit} onDelete={onDelete} />
              ))}
            </tbody>
          </table>
        </div>
